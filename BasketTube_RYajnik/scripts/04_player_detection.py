import cv2
from ultralytics import YOLO
from pathlib import Path
import json
from tqdm import tqdm

# ========================= CONFIG =========================
CHUNKS_DIR = "outputs/clips"
FRAMES_DIR = "outputs/frames"
DETECTIONS_DIR = Path("outputs/results/detections")
MODEL_PATH = "yolov8x.pt"
CONF_THRESHOLD = 0.30
MAX_CHUNKS = 20                    # Keep low for now
# =======================================================

DETECTIONS_DIR.mkdir(parents=True, exist_ok=True)
Path(FRAMES_DIR).mkdir(parents=True, exist_ok=True)

print("Loading YOLOv8x model...")
model = YOLO(MODEL_PATH)

chunk_files = sorted(list(Path(CHUNKS_DIR).glob("*.mp4")))[:MAX_CHUNKS]

print(f"Processing {len(chunk_files)} chunks...")

all_detections = []
total_frames_processed = 0

for chunk_path in tqdm(chunk_files, desc="Processing chunks"):
    chunk_name = chunk_path.stem
    chunk_frame_dir = Path(FRAMES_DIR) / chunk_name
    chunk_frame_dir.mkdir(exist_ok=True)
    
    cap = cv2.VideoCapture(str(chunk_path))
    fps = cap.get(cv2.CAP_PROP_FPS)
    frame_count = 0
    detections_in_chunk = 0
    
    while True:
        ret, frame = cap.read()
        if not ret:
            break
            
        total_frames_processed += 1
        
        # Sample ~ every 1.5 seconds
        if frame_count % max(1, int(fps * 1.5)) == 0:
            # Run detection on ALL objects (no class filter)
            results = model(frame, conf=CONF_THRESHOLD, verbose=False)
            
            # Save frame
            frame_path = chunk_frame_dir / f"frame_{frame_count:06d}.jpg"
            cv2.imwrite(str(frame_path), frame)
            
            for box in results[0].boxes:
                x1, y1, x2, y2 = map(int, box.xyxy[0].cpu().numpy())
                conf = float(box.conf[0])
                cls = int(box.cls[0])
                
                all_detections.append({
                    "chunk": chunk_name,
                    "frame": frame_count,
                    "bbox": [x1, y1, x2, y2],
                    "confidence": round(conf, 3),
                    "class": cls,
                    "frame_path": str(frame_path.relative_to("outputs"))
                })
                detections_in_chunk += 1
        
        frame_count += 1
    
    cap.release()
    print(f"  → {chunk_name}: {detections_in_chunk} detections")

# Save results
with open(DETECTIONS_DIR / "player_detections.json", "w") as f:
    json.dump(all_detections, f, indent=2)

print(f"\n🎉 Player Detection Completed!")
print(f"   • Chunks processed : {len(chunk_files)}")
print(f"   • Total detections : {len(all_detections)}")
print(f"   • Frames saved     : {FRAMES_DIR}")
